...
def load_data(sample_size=None):
    ...
    if sample_size:
        df = df.sample(sample_size, random_state=42)  # Load a subset of data
    return df
...
# Cache SHAP value computation
@st.cache_data
def get_shap_values(model, data, sample_size=1000):
    explainer = shap.Explainer(model, data[:sample_size], algorithm="auto")
    shap_values = explainer(data[:sample_size])
    return shap_values
...
df = load_data(sample_size=10000)  # Use a 10,000-row sample for faster loading
model = load_model()
...
X_shap_sampled = df[FINAL_FEATURES].sample(1000, random_state=42)  # Use 1000 rows for SHAP
shap_values = get_shap_values(model, X_shap_sampled)
